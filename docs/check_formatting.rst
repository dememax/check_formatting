...
One supported install route per backend — see each backend's own paragraph
above for alternatives and gotchas:
...
.. list-table::
   ...
   * - ``meson``
     ...
     - ``pip install meson`` — also needs a ``meson.format`` file at the
       project root, even empty (see above)
   * - ``web``, ``json``, ``ini``, ``yaml``
     - prettier
   ...
   * - ``vnu``
     - Nu Html Checker
     - No package-manager one-liner — pinned-version install procedure
       above, into ``~/opt``
...
Inverse of the Registered checkers table above — keyed by what's already
in the repository, not by checker name:
...
above); ``clang-format``, ``meson``, ``ruff``, and ``mypy`` on ``PATH``;
...
.. code-block:: markdown
   ...
   outstanding violations. See [docs/check_formatting.rst's "Continuous
   integration" section](docs/check_formatting.rst#continuous-integration)
